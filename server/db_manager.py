...
class database:
    def __init__(self):
        ...
        self.engine = create_engine('sqlite:///'+script_path+'/dht_database.db', echo=True)
    ...
    def create_agent(self, agent_name):
        if not self.engine.dialect.has_table(self.engine, agent_name):
            agent_table = Table(agent_name, self.metadata, Column('id', Integer, primary_key=True, autoincrement=True), Column('humidity', Integer), Column('temperature',Integer), Column('date_time', Date), autoload=True)
...
if __name__ == "__main__":
    ...
    print(results)
    print("Running Test for if nothing in DB")
